...
dirName = 'C:/Users/Hans Thoft Rasmussen/Documents/Aalborg University/ROB6/Project/Machine Learning/dataset/test'
...
def display_selected(choice):
    ...
    if mask.shape[2] != 0:
        for i in range(len(list(filtered))):
            ...
            refCoord = [int(rX), int(rY)]
            absCoord = [int(cX), int(cY)]

            points.append(absCoord)
    ...
    dim = (width, height)
    try:
        tp.config(text=str(points[0]))
        rp.config(text=str(refpoints[0]))
    except:
        pass

    # resize image
    #resized = cv2.resize(backtorgb, dim, interpolation=cv2.INTER_AREA)
    backtorgb = backtorgb[ : , 215:1190+315]
...
empty = np.zeros((1080, 1290, 3), dtype=np.uint8)
...
label = tk.Label(window, text='Target point:', bg='#000000' ,fg='white', font=("Samsung Sharp Sans", 12, "bold"), bd=0)
label.place(anchor='c', x=430, y=200, width=200, height=50)

tp = tk.Label(window, text='No Target', bg='#000000' ,fg='white', font=("Samsung Sharp Sans", 12, "bold"), bd=0)
tp.place(anchor='c', x=430, y=240, width=200, height=50)

rp = tk.Label(window, text='No Reference', bg='#000000' ,fg='white', font=("Samsung Sharp Sans", 12, "bold"), bd=0)
rp.place(anchor='c', x=430, y=280, width=200, height=50)

label = tk.Label(window, text='Robot Control', bg='#5c31ad' ,fg='white', font=("Samsung Sharp Sans", 14, "bold"), bd=0)
label.place(anchor='c', x=1920-160, y=50, width=220, height=25)
...
def mode():
    ...
    if manual == 1:
        manual = 0
        print("Manual ON")
        man.config(text="MANUAL")
    else:
        manual = 1
        print("Manual OFF")
        man.config(text="AUTO")
...
button = tk.Button(window, text="MODE", command=mode, bg='black', fg='white', font=("Samsung Sharp Sans", 14, "bold"), bd=0)
button.place(anchor='c', x=1920-160, y=280, width=220, height=50)

button = tk.Button(window, text="RUN", command=run, bg='#44d460', fg='white', font=("Samsung Sharp Sans", 14, "bold"), bd=0)
button.place(anchor='c', x=1920-160, y=280+110, width=220, height=50)

man = tk.Label(window, text='MANUAL', bg='black' ,fg='white', font=("Samsung Sharp Sans", 14, "bold"), bd=0)
man.place(anchor='c', x=1920-160, y=280+50, width=220, height=50)
